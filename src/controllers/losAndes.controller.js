...
exports.validar = async (req, res, next) => {
    try {
        ...
        // De acuerdo a la documentación de Postman, retorna el tipo de afiliación.
        // Asumiremos que si la API responde sin error, es porque existe una afiliación válida.
        // Si no fuera afiliado, usualmente estas APIs devuelven un código de estado o un campo específico.
        
        // --- PROCESAR RESULTADO --- (Ejemplo: resultadoExterno.tipoAfiliacion o similar)
        // Por ahora, si llegamos aquí sin error de BusinessError del service, procedemos.

        // 2. Buscar o Crear Pasajero en base al RUT completo
        let [pasajero] = await Pasajero.findOrCreate({
            where: { rut: rut },
            defaults: {
                nombres: 'Afiliado', // Valor temporal si la API no devuelve el nombre
                apellidos: 'Los Andes',
                status: 'ACTIVO'
            }
        });
