const { ApiRegistro } = require('../models');
const NotFoundError = require('../exceptions/NotFoundError');
const { getPagination, getPagingData } = require('../utils/pagination.utils');
const { Op } = require('sequelize');

exports.crear = async (data) => {
    return await ApiRegistro.create(data);
};
...
exports.listar = async (params) => {
    ...
    const data = await ApiRegistro.findAndCountAll({
        where,
        limit: l,
        offset,
...
exports.obtenerPorId = async (id) => {
    const api = await ApiRegistro.findByPk(id);
    if (!api) throw new NotFoundError('API de registro no encontrada');
    return api;
};
